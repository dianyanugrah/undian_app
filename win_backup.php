...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if($_POST['action'] == 'undian2') {
		...
		$query_rand_1 = "SELECT users_tb.id_user, users_tb.full_name, users_tb.no_id_ktp, users_tb.no_telp, users_tb.kota, users_tb.alamat, kode_tb.no_trx, kode_tb.kode_input
            ...
            WHERE users_tb.status_user = 1 AND win = 0 AND users_tb.no_id_ktp REGEXP '^[0-9]+$' AND users_tb.no_telp REGEXP '^[0-9]+$'
		...
		foreach ($pemenang_list as $key => $value) {
			$save_win_query = "INSERT INTO pemenang_tb (id_peserta,undian_ke,datetime_winner,stat) VALUES (".$value['id_user'].",2,'".date('Y-m-d H:i:s')."', 1)";
			$save_win_exec = mysql_query($save_win_query);

			if($save_win_exec) {
				$winner_win_query = "UPDATE users_tb SET status_user = 2, win = 1, win_datetime = '".date('Y-m-d H:i:s')."' WHERE no_id_ktp = ".$value['no_id_ktp']."";
	...
	} else if($_POST['action'] == 'show_winner_req') {
		...
		// PEMENANG UNDIAN KE 2
		// $query_rand = "SELECT pem.id, pes.id_peserta, pes.nama, pes.alamat, pes.kota, pes.no_ktp, pes.no_ktp, pes.no_telpon, pes.kode_struk, pes.outlet FROM fiesta_pemenang pem INNER JOIN fiesta_peserta pes ON pem.id_peserta = pes.id_peserta WHERE pem.undian_ke = 2 AND pem.stat = 1";
		$query_rand = "SELECT pem.id, users_tb.id_user, users_tb.full_name, users_tb.no_id_ktp, users_tb.no_telp, users_tb.kota, users_tb.alamat, kode_tb.no_trx, kode_tb.kode_input
            ...
            INNER JOIN kode_tb ON users_tb.no_id_ktp = kode_tb.no_id_ktp 
            WHERE users_tb.status_user = 2 AND users_tb.no_id_ktp REGEXP '^[0-9]+$' AND users_tb.no_telp REGEXP '^[0-9]+$' GROUP BY users_tb.no_id_ktp";
	...
	} else if($_POST['action'] == 'show_winner_req1') {
		...
		// $query_data = "SELECT pes.id_peserta, pes.nama, pes.alamat, pes.kota, pes.no_ktp, pes.no_ktp, pes.no_telpon, pes.kode_struk, pes.outlet FROM fiesta_pemenang pem INNER JOIN fiesta_peserta pes ON pem.id_peserta = pes.id_peserta";
		...
		$query_data = "SELECT pem.id, users_tb.id_user, users_tb.full_name, users_tb.no_id_ktp, users_tb.no_telp, users_tb.kota, users_tb.alamat, kode_tb.no_trx, kode_tb.kode_input
            ...
            INNER JOIN pemenang_tb pem ON pem.id_peserta = users_tb.id_user
            INNER JOIN kode_tb ON users_tb.no_id_ktp = kode_tb.no_id_ktp 
            WHERE pem.show_ajax = 0 AND users_tb.status_user = 2 AND users_tb.no_id_ktp REGEXP '^[0-9]+$' AND users_tb.no_telp REGEXP '^[0-9]+$'  GROUP BY no_id_ktp ORDER BY RAND() LIMIT 1";
