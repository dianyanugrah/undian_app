...
$dataPemenangQuery = "SELECT pem.id, users_tb.id_user, users_tb.full_name, users_tb.no_id_ktp, users_tb.no_telp, users_tb.kota, users_tb.alamat, kode_tb.no_trx, kode_tb.kode_input
            ...
            WHERE pem.jenis_undian = 2 AND pem.jenis_hadiah = 1 AND pem.stat = 1 AND users_tb.no_id_ktp REGEXP '^[0-9]+$' AND users_tb.no_telp REGEXP '^[0-9]+$' GROUP BY users_tb.no_id_ktp";
...
if(isset($_GET['q'])) {
	...
	if($_GET['q'] == 'o') {
		...
		$update_stat_pemenang = "UPDATE pemenang_tb SET stat = 2 WHERE jenis_hadiah = 1 AND jenis_undian = 2";
		...
		if($_GET['u'] == 1) {
			// echo "Undian 1;
			$filename = "Data_Pemenang_Gopay_Single_Draw".date('YmdHis').".xlsx";
			$rows = $listNew;
		}
	} else if($_GET['q'] == 'ek') {
		...
		<div class="main-area center-text" style="background-image:url(assets/images/gambar3.png);">
			...
			<div class="display-table">
				<div class="display-table-cell">
					...
					<h3 class="title"><b>GOPAY VOUCHER</b></h3>
					...
					<ul id="tools" class="social-btn">
						...
						<li><a id="singledraw" style="background-color: #FF8C00; padding-right: 15px" href="multi_draw_gopay.php"><i class="ion-shuffle"></i>Multi Draw Gopay</a></li>
				...
				<div class="display-table-cell">
					...
					<table class="table table-bordered">
					    ...
					    <tbody id="export_btn" style="display: none">
					      	<tr>
					      		<td colspan="7">
					      			<a href="single_draw_gopay.php?q=p&u=1" id="export_winner" style="background-color: #DAB000; color: #fff; padding: 15px"><i class="ion-android-arrow-down"></i> Export Data Pemenang</a>
					      		</td>
					      	</tr>
					    </tbody>
				  	</table>
							...
							<div class="col-md-12">
								<div class="panel panel-primary">
								  	...
								  	<div class="panel-footer">
								  		<a href="single_draw_gopay.php?q=ek" id="export_kota" style="background-color: #DAB000; color: #fff; padding: 15px"><i class="ion-android-arrow-down"></i> Export Data</a>
								  	</div>
								</div>
							</div>								
						</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			if(flagWinner > 0) {
				...
				$.ajax({ 
			        url: 'win_gopay.php',
			        data: {"action": "show_single_winner"},
			        type: 'post',
		...
		function show_winner() {
			$.ajax({ 
		        url: 'win_gopay.php',
		        data: {"action": "show_single_winner1"},
		        type: 'post',
		        success: function(result) {
	            	if(result.process === 1) {
	            		if(result.jumlah === 1) {
        ...
        $('#reset').click(function() {
        	if(flagWinner === 1) {
	        	if (confirm('Anda ingin memulihkan data pemenang undian ?')) {
				    window.location.href = "single_draw_gopay.php?q=r";
        ...
        $('#draw_stop').click(function() {
			...
			$.ajax({ 
		        url: 'win_gopay.php',
		        data: {"action": "single_draw", "limit": limit},
		        type: 'post',
		        success: function(result) {
		        	console.log(result)
	            	if(result.process === 1) {
