...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if($_POST['action'] == 'single_draw') {
		...
		foreach ($pemenang_list as $key => $value) {
			$save_win_query = "INSERT INTO pemenang_tb (id_peserta,jenis_hadiah,jenis_undian, datetime_winner,stat) VALUES (".$value['id_user'].",1,2,'".date('Y-m-d H:i:s')."', 1)";
		...
		$query_data = "SELECT pem.id, users_tb.id_user, users_tb.full_name, users_tb.no_id_ktp, users_tb.no_telp, users_tb.kota, users_tb.alamat, kode_tb.no_trx, kode_tb.kode_input
            ...
            INNER JOIN pemenang_tb pem ON pem.id_peserta = users_tb.id_user
            INNER JOIN kode_tb ON users_tb.no_id_ktp = kode_tb.no_id_ktp 
            WHERE pem.show_ajax = 0 AND pem.jenis_undian = 2 AND pem.jenis_hadiah = 1 AND pem.stat = 1 AND users_tb.no_id_ktp REGEXP '^[0-9]+$' AND users_tb.no_telp REGEXP '^[0-9]+$'  GROUP BY no_id_ktp ORDER BY RAND() LIMIT 1";
		...
		//EXEC TO DB
		foreach ($pemenang_list as $key => $value) {
			$save_win_query = "INSERT INTO pemenang_tb (id_peserta,jenis_hadiah,jenis_undian, datetime_winner,stat) VALUES (".$value['id_user'].",1,1,'".date('Y-m-d H:i:s')."', 1)";
			$save_win_exec = mysql_query($save_win_query);
		...
		$query_data = "SELECT pem.id, users_tb.id_user, users_tb.full_name, users_tb.no_id_ktp, users_tb.no_telp, users_tb.kota, users_tb.alamat, kode_tb.no_trx, kode_tb.kode_input
            ...
            INNER JOIN pemenang_tb pem ON pem.id_peserta = users_tb.id_user
            INNER JOIN kode_tb ON users_tb.no_id_ktp = kode_tb.no_id_ktp 
            WHERE pem.show_ajax = 0 AND pem.jenis_undian = 1 AND pem.jenis_hadiah = 1 AND pem.stat = 1 AND users_tb.no_id_ktp REGEXP '^[0-9]+$' AND users_tb.no_telp REGEXP '^[0-9]+$'  GROUP BY no_id_ktp ORDER BY RAND() LIMIT 1";
		$sql_data = mysql_query($query_data);
		while ($rows_data = mysql_fetch_assoc($sql_data)) {
			$pemenang_data[] = $rows_data;
		}
